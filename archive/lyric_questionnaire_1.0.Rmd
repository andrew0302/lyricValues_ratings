...
```{r}
ui <- fluidPage(
  titlePanel("Lyric Questions Distributions"),
...
```{r}
ui <- fluidPage(
  titlePanel("Lyric Questions Correlations"),
...
server <- function(input, output, session){
  ...
  # correlation plot of lyrics responses
  output$corrplot <- renderPlot({
  corrplot(cor(corrplot_dt), method = "color", type = 'lower', diag=FALSE, addCoef.col="black", p.mat = p.mat, sig.level=0.05)
  })
