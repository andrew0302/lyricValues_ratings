...
Examining the time taken for the intial lyrics questionnaire and per song:
...
Both prolific and qualtrics have their own measures of time taken for the entire survey. 
...
time_taken_plot <- responses_dt[!Sex=='CONSENT REVOKED'] %>% .[time_taken < 100000] %>%select(columns) %>%
  ...
  scale_x_continuous(limits = c(1000, 12500))
time_taken_plot
...
rm(first, last, clicks_dt)
```

Here I plot the time taken in seconds for each question block. 
Two observations appear to be clear outliers. 
At 9000+ seconds, this is likely an issue with recording the time taken perhaps by the Qualtrics platform itself. 
...
both[order(-diff),]
```
The mean time taken is 44.76 seconds, with an SD of 101.7. 
Removing outliers above 900 seconds, gives a mean of 42.76, with an SD of 39.53. 

Thus I conclude that a conservative time estimate per stimulus rating is 83 = 43 + 40 seconds. 
...
No reason to really expext sex differences
```{r}
...
both_outlier <- both[diff<165 & diff > 15]
adjbox(both_outlier$diff)
```



Here I build a data table of the mean time taken per question block, excluding outlier defined as > 900 seconds. 
