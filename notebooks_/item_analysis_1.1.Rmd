...
Number of Raters Estimation:

This notebook creates visualizations of results for individual items. 
The two primary displays show:
* Per Value distribution of ratings per song
* Per Song distribution of ratings per value
* Per Song distribution of ratings per value along with confidence ratings
...
source(here("scripts_", "data_file_re-shape_1.0.R"))
```
...
```{r}
test_dt <- responses_dt %>% select(-participant_ID, -item_ID)
```

```{r}
mvn(test_dt, mvnTest=c("mardia"))
mvn(test_dt, mvnTest=c("hz"))
mvn(test_dt, mvnTest=c("dh"))
```
